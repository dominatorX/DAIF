...
public class get_info_euc {
    static int norm(double[] x, double[] y) {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while ((str1 = Yedges.readLine()) != null) {
            ...
            Edge.add(source);
            Edge.add(des);
            double[] locs = meter2id.get(source);
            double[] locd = meter2id.get(des);
            //int weight = (int)(Double.parseDouble(str1.split(",")[4])*60);
            int weight = (int)Math.ceil(Double.max(Double.parseDouble(str1.split(",")[2]), norm(locs, locd))/speed);
            // a real metric map
            if (weight==0){
                Edge.add(1);
            }else {
